...
import { isShoppingBagOpenSelector, showCheckout } from "../../redux/app"
import {
  checkout,
      ...
      <div className={classes.root}>
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className={classes.container}>
              {items.map((id) => (
                <ShoppingBagItem id={id} key={`item-${id}`} />
              ))}
              <div className={classes.subtotalRow}>
                <Typography variant="h6" className={classes.amountLabel}>
                  Subtotal:
                </Typography>
                <Typography variant="h6" className={classes.amount}>
                  ${subtotalAmount}{" "}
                  <Typography component="span" variant="body2">
                    {props.currency ? props.currency : "CAD"}
                  </Typography>
                </Typography>
              </div>
              ...
              <div className={classes.totalAmountRow}>
                <Typography
                  variant="h6"
                  className={`${classes.amountLabel} ${classes.totalAmountLabel}`}
                >
                  Total:
                </Typography>
                <Typography
                  variant="h6"
                  className={`${classes.amount} ${classes.totalAmount}`}
                >
                  ${totalAmount}{" "}
                  <Typography component="span" variant="body2">
                    {props.currency ? props.currency : "CAD"}
